  ...
  async getOsuFile(options: OsuFileOptions): Promise<MirrorResult<Buffer>> {
    ...
    if (options.expectedMd5) {
      ...
      if (hash !== options.expectedMd5) {
        console.log(`[direct] MD5 mismatch: beatmap=${options.beatmapId}, expected=${options.expectedMd5}, actual=${hash}`);
        return {
          success: false,
          data: null,
          error: "MD5 mismatch",
      ...
      console.log(`[direct] MD5 match: beatmap=${options.beatmapId}, hash=${hash}`);
    }
